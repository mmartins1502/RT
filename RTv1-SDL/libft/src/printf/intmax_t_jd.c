...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/12/02 02:07:28 by nsampre           #+#    #+#             */
...
int		ft_display_intmax_t(unsigned long long int nb, int display)
{
	int						count;
...
int		ft_put_intmax_t(long long int nb, int display, t_format *format)
{
	int	count;
	...
	if (nb < 0)
	{
		...
		IS_NEGATIVE = 0;
	return (count += ft_display_intmax_t(nb, display));
}

void	ft_intmax_t_get_central_len(intmax_t jd, t_format *format)
{
	LEN = ft_put_intmax_t(jd, 0, format);
